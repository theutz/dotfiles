...
(defun my-nushell-tangle-formatter ()
    "Format BODY of each tangled code block."
    (save-excursion
      (goto-char (point-min))
...
(add-hook! 'org-babel-post-tangle-hook #'my-nushell-tangle-formatter)
#+end_src
...
(after! org
  ...
  (map! :localleader :mode org-mode :desc "org-columns" :n "m" #'org-columns)
...
And some things can run on a hook!
