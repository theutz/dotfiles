...
#+begin_src nushell :tangle no :noweb-ref nu-scripts
...
First, we'll want to make sure our base nushell configuration is using the module.

#+begin_src :tangle no :noweb-ref nu-modules
overlay use --prefix wm
#+end_src

Now we'll create a =mod.nu= file that will be the central hub where the commands are loaded.
...
:header-args:nushell: :tangle (expand-file-name "wm/mode.nu" (org-sbe nu-primary-lib-dir)) :comments link
:END:

We're going to organize our keybindings around modes in SKHD. So, we'll setup a small module to help us. And we'll make sure it's imported in the root module.

#+begin_src nushell :tangle no :noweb-ref wm-exports
export use mode.nu
#+end_src

**** Modes

Here we'll create helper functions for actually switching to distinct modes.
...
Additionally, we'll setup some functions that will standardize the process of moving between modes.

***** go-back

This will pop you back a single level--usually to =yab= mode.
...
This should kick the user all the way back to =default= mode so they can use their keyboard normally.
